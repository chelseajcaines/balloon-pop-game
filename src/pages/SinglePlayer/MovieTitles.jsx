...
const SinglePlayerGamePlay = () => {
  ...
  const [isHomePageButtonClicked, setIsHomePageButtonClicked] = useState(false);
  ...
  const fetchPuzzle = async () => {
    ...
    try {
      ...
      const randomPuzzleIndex = Math.floor(Math.random() * data.items.length);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (showLeaveGameModal && isHomePageButtonClicked && e.key === "Enter") {
        const currentIndex = leaveGameModalbuttons.findIndex(
          (button) => button.id === isActive
        );
          ...
          {showLeaveGameModal && (
            <div>
              <div
                style={overlayStyles}
                onClick={handleCancelAllModals}
                className={showLeaveGameModal ? styles.overlay : ""}
              ></div>
              <div
                style={modalStyles}
                className={showLeaveGameModal ? styles.modal : ""}
              >
